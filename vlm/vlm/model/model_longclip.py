...
class CLIP(nn.Module):
    ...
    def forward(self, images, longtexts, shorttexts ,rank):
        image_features_long = self.encode_image(images)
        text_features_long = self.encode_text(longtexts)
        text_features_short = self.encode_text(shorttexts)
        ...
        # image_features_short = self.PCA(image_features_long, 64)
        image_features_short = image_features_long

         # 目标张量列表
        world_size = torch.distributed.get_world_size()
        gather_list_image_long = [torch.zeros_like(image_features_long) for _ in range(world_size)]
        gather_list_image_short = [torch.zeros_like(image_features_short) for _ in range(world_size)]
        gather_list_text_long = [torch.zeros_like(text_features_long) for _ in range(world_size)]
        gather_list_text_short = [torch.zeros_like(text_features_short) for _ in range(world_size)]

       # 收集所有进程的数据
        torch.distributed.all_gather(gather_list_image_long, image_features_long)
        torch.distributed.all_gather(gather_list_image_short, image_features_short)
        torch.distributed.all_gather(gather_list_text_long, text_features_long)
        torch.distributed.all_gather(gather_list_text_short, text_features_short)

        # 将收集到的数据进行连接
        image_feat_all_long = torch.cat(gather_list_image_long, dim=0)
        image_features_all_short = torch.cat(gather_list_image_short, dim=0)
        text_feat_all_long = torch.cat(gather_list_text_long, dim=0)
        text_feat_all_short = torch.cat(gather_list_text_short, dim=0)
        ...
        loss_itcl = (
                F.cross_entropy(sim_i2tl, targets)
                + F.cross_entropy(sim_tl2i, targets)
            ) / 2
